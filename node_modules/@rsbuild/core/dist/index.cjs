...
async function pathExists(path21) {
  return import_node_fs.default.promises.access(path21).then(() => true).catch(() => false);
}
...
function formatStats(statsData, hasErrors) {
  const verbose = import_rslog.logger.level === "verbose";
  if (hasErrors) {
    ...
    hintNodePolyfill = (message) => {
      const getTips = (moduleName2) => {
        const tips = [
          ...
          `Check if you need to import Node.js module. If needed, you can use ${import_picocolors2.default.cyan("@rsbuild/plugin-node-polyfill")}.`
        ];
        return `${message}
...
var require_main = __commonJS({
  "../../node_modules/.pnpm/dotenv@16.4.5/node_modules/dotenv/lib/main.js"(exports, module2) {
    ...
    function _resolveHome(envPath) {
      return envPath[0] === "~" ? path21.join(os2.homedir(), envPath.slice(1)) : envPath;
    }
    ...
    function configDotenv(options) {
      ...
      for (const path22 of optionPaths) {
        try {
          const parsed = DotenvModule.parse(fs12.readFileSync(path22, { encoding }));
...
function isFile(file) {
  ...
  const result = import_node_fs4.default.existsSync(file) && import_node_fs4.default.statSync(file).isFile();
...
function parsePackage(file) {
  const config = JSON.parse(
    import_node_fs4.default.readFileSync(file).toString().replace(/^\uFEFF/m, "")
  );
...
function readConfig(file) {
  ...
  return parseConfig(import_node_fs4.default.readFileSync(file, "utf-8"));
}
function parsePackageOrReadConfig(file) {
  if (import_node_path8.default.basename(file) === "package.json") {
...
function eachParent(file, callback) {
  const dir = isFile(file) ? import_node_path8.default.dirname(file) : file;
  let loc = import_node_path8.default.resolve(dir);
  do {
    const result = callback(loc);
    if (typeof result !== "undefined")
      return result;
  } while (loc !== (loc = import_node_path8.default.dirname(loc)));
  return void 0;
}
function findConfigFile(from) {
  return eachParent(from, (dir) => {
    const config = import_node_path8.default.join(dir, "browserslist");
    const pkg = import_node_path8.default.join(dir, "package.json");
    const rc = import_node_path8.default.join(dir, ".browserslistrc");
    let pkgBrowserslist;
    if (isFile(pkg)) {
      try {
...
function findConfig(from) {
  from = import_node_path8.default.resolve(from);
  const fromDir = isFile(from) ? import_node_path8.default.dirname(from) : from;
  ...
  const configDir = configFile && import_node_path8.default.dirname(configFile);
  eachParent(from, (dir) => {
    if (resolved) {
...
function getHTMLPathByEntry(entryName, config) {
  ...
  return removeLeadingSlash(import_node_path9.posix.join(prefix, filename));
}
  ...
  const getTransformFn = (environment) => (descriptor, handler) => {
    ...
    hooks.modifyBundlerChain.tapEnvironment({
      ...
      handler: (chain, { target, environment: environment2 }) => {
        ...
        const loaderPath = (0, import_node_path9.join)(LOADER_PATH, loaderName);
...
var import_node_path9, mapProcessAssetsStage;
...
async function getBrowserslist(path21) {
  ...
  const result = loadConfig2({ path: path21, env });
  if (result) {
...
var import_node_path10, browsersListCache, getEnvironmentHTMLPaths;
...
var import_node_path11, getInspectOutputPath;
  ...
  "src/provider/inspectConfig.ts"() {
    ...
    getInspectOutputPath = (context, inspectOptions) => {
      if (inspectOptions.outputPath) {
        if ((0, import_node_path11.isAbsolute)(inspectOptions.outputPath)) {
          return inspectOptions.outputPath;
        }
        return (0, import_node_path11.join)(context.distPath, inspectOptions.outputPath);
      }
      return (0, import_node_path11.join)(context.distPath, RSBUILD_OUTPUTS_PATH);
    };
  }
});
...
var import_node_path12, isRelativePath, getSubPath, BasicRunner;
    ...
    BasicRunner = class {
      ...
      getFile(modulePath, currentDirectory) {
        if (Array.isArray(modulePath)) {
          return {
            path: import_node_path12.default.join(currentDirectory, ".array-require.js"),
        ...
        if (isRelativePath(modulePath)) {
          const p = import_node_path12.default.join(currentDirectory, modulePath);
          return {
            path: p,
...
var import_node_path13, import_node_vm2, define, CommonJsRunner;
var init_cjs = __esm({
  "src/server/runner/cjs.ts"() {
    "use strict";
    ...
    CommonJsRunner = class extends BasicRunner {
      ...
      createModuleScope(requireFn, m, file) {
        return {
          ...
          require: requireFn.bind(null, import_node_path13.default.dirname(file.path)),
          module: m,
          exports: m.exports,
          __dirname: import_node_path13.default.dirname(file.path),
...
var import_node_path14, import_node_url, import_node_vm3, EsmRunner;
var init_esm2 = __esm({
  "src/server/runner/esm.ts"() {
    "use strict";
          ...
          if (!esm) {
            esm = new import_node_vm3.default.SourceTextModule(file.content, {
              ...
              importModuleDynamically: async (specifier, module2) => {
                const result = await _require(import_node_path14.default.dirname(file.path), specifier, {
                  esmMode: 1 /* Evaluated */
                });
          ...
          return (async () => {
            await esm.link(async (specifier, referencingModule) => {
              return await asModule(
                await _require(
                  import_node_path14.default.dirname(
...
var import_node_path15, loadBundle, getTransformedHtml;
var init_environment = __esm({
  "src/server/environment.ts"() {
    ...
    getTransformedHtml = async (entryName, utils) => {
      ...
      const fileName = (0, import_node_path15.join)(distPath, htmlPath);
...
async function createDevServer(options, createCompiler2, config, {
  ...
  let outputFileSystem = import_node_fs5.default;
  let lastStats;
  ...
  const startCompile = async () => {
    ...
    outputFileSystem = (isMultiCompiler(compiler) ? compiler.compilers[0].outputFileSystem : compiler.outputFileSystem) || import_node_fs5.default;
    return {
  ...
  const readFileSync = (fileName) => {
    ...
    return import_node_fs5.default.readFileSync(fileName, "utf-8");
  };
...
var import_node_fs5, formatDevConfig;
...
var import_node_path19, getJsSourceMap, pluginBasic;
          ...
          (chain, { env, isDev, target, bundler, environment, CHAIN_ID: CHAIN_ID2 }) => {
            ...
            if (env === "development") {
              chain.output.devtoolModuleFilenameTemplate(
                (info) => import_node_path19.default.resolve(info.absoluteResourcePath).replace(/\\/g, "/")
              );
            }
...
async function validateCache(cacheDirectory, buildDependencies) {
  const configFile = (0, import_node_path20.join)(cacheDirectory, "buildDependencies.json");
  if (await isFileExists(configFile)) {
    const rawConfigFile = await import_node_fs6.default.promises.readFile(configFile, "utf-8");
    ...
    await import_node_fs6.default.promises.rm(cacheDirectory, { force: true, recursive: true });
  }
  await import_node_fs6.default.promises.mkdir(cacheDirectory, { recursive: true });
  await import_node_fs6.default.promises.writeFile(configFile, JSON.stringify(buildDependencies));
}
...
function getCacheDirectory({ cacheDirectory }, context) {
  if (cacheDirectory) {
    return (0, import_node_path20.isAbsolute)(cacheDirectory) ? cacheDirectory : (0, import_node_path20.join)(context.rootPath, cacheDirectory);
  }
  return (0, import_node_path20.join)(context.cachePath, context.bundlerType);
}
async function getBuildDependencies(context, config, environmentContext) {
  const rootPackageJson = (0, import_node_path20.join)(context.rootPath, "package.json");
  const browserslistConfig = (0, import_node_path20.join)(context.rootPath, ".browserslistrc");
  ...
  const configs = tailwindExts.map(
    (ext) => (0, import_node_path20.join)(context.rootPath, `tailwind.config.${ext}`)
  );
...
var import_node_crypto, import_node_fs6, import_node_path20, pluginCache;
...
async function applyCSSRule({
  ...
  if (target === "web") {
    ...
    rule.use(CHAIN_ID2.USE.IGNORE_CSS).loader(import_node_path21.default.join(LOADER_PATH, "ignoreCssLoader.cjs"));
  }
  let importLoaders = 0;
...
var import_node_path22, import_core5, getJsAsyncPath, pluginOutput;
          ...
          async (chain, { CHAIN_ID: CHAIN_ID2, target, isProd, isServer, environment }) => {
            ...
            chain.output.path(distPath).filename(
              isJsFilenameFn ? (...args) => {
                ...
                return import_node_path22.posix.join(jsPath, name);
              } : import_node_path22.posix.join(jsPath, jsFilename)
            ).chunkFilename(
              isJsFilenameFn ? (...args) => {
                const name = jsFilename(...args);
                return import_node_path22.posix.join(jsAsyncPath, name);
              } : import_node_path22.posix.join(jsAsyncPath, jsFilename)
...
async function printFileSizes(options, stats, rootPath, environmentName) {
  ...
  const formatAsset = async (asset, distPath, distFolder) => {
    ...
    const contents = await import_node_fs7.default.promises.readFile(import_node_path23.default.join(distPath, fileName));
    ...
    return {
      size,
      folder: import_node_path23.default.join(distFolder, import_node_path23.default.dirname(fileName)),
      name: import_node_path23.default.basename(fileName),
      gzippedSize,
  ...
  const getAssets = async () => {
    ...
    const distFolder = import_node_path23.default.relative(rootPath, distPath);
    return Promise.all(
  ...
  for (const asset of assets) {
    ...
    const fileNameLength = (folder + import_node_path23.default.sep + name).length;
...
var init_cleanOutput = __esm({
  "src/plugins/cleanOutput.ts"() {
    ...
    init_helpers();
    init_logger();
    addTrailingSep = (dir) => dir.endsWith(import_node_path24.sep) ? dir : dir + import_node_path24.sep;
    ...
    pluginCleanOutput = () => ({
      ...
      setup(api) {
        const getRsbuildCleanPath = () => {
          ...
          const cleanPath = (0, import_node_path24.join)(distPath, RSBUILD_OUTPUTS_PATH);
...
var import_node_path25, chainStaticAssetRule, pluginAsset;
...
var import_node_path26, import_node_util3, VOID_TAGS, HEAD_TAGS, FILE_ATTRS, hasTitle, getTagPriority, formatBasicTag, fromBasicTag, formatTags, applyTagConfig, addTitleTag, RsbuildHtmlPlugin;
    ...
    RsbuildHtmlPlugin = class {
      ...
      apply(compiler) {
        const emitFavicon = async (compilation, favicon) => {
          const name = import_node_path26.default.basename(favicon);
          ...
          const filename = import_node_path26.default.resolve(compilation.compiler.context, favicon);
...
async function getTemplate(entryName, config, rootPath) {
  ...
  const absolutePath = (0, import_node_path27.isAbsolute)(templatePath) ? templatePath : import_node_path27.default.resolve(rootPath, templatePath);
...
var import_node_path28, import_node_util4, import_mrmime, pluginAppIcon;
    ...
    pluginAppIcon = () => ({
      ...
      setup(api) {
        ...
        const formatIcon = (icon, distDir, publicPath) => {
          ...
          const absolutePath = import_node_path28.default.isAbsolute(src) ? src : import_node_path28.default.join(api.context.rootPath, src);
          const relativePath = import_node_path28.default.posix.join(
            distDir,
            import_node_path28.default.basename(absolutePath)
          );
...
var import_node_path29, pluginWasm;
var init_wasm = __esm({
  "src/plugins/wasm.ts"() {
    "use strict";
    ...
    pluginWasm = () => ({
      ...
      setup(api) {
        api.modifyBundlerChain(async (chain, { CHAIN_ID: CHAIN_ID2, environment }) => {
          ...
          const wasmFilename = import_node_path29.posix.join(distPath, "[hash].module.wasm");
          chain.output.merge({
...
var import_node_path30, getFilename2, pluginNodeAddons;
var init_nodeAddons = __esm({
  "src/plugins/nodeAddons.ts"() {
    ...
    getFilename2 = (resourcePath) => {
      let basename = "";
      if (resourcePath) {
        const parsed = import_node_path30.default.parse(resourcePath);
        if (parsed.dir) {
...
async function applyCoreJs(swcConfig, polyfillMode) {
  ...
  const coreJsDir = import_node_path31.default.dirname(coreJsPath);
...
var import_node_fs9, import_node_path31, import_deepmerge4, builtinSwcLoaderName, pluginSwc, getCoreJsVersion, reduceTransformImportConfig;
var init_swc = __esm({
  "src/plugins/swc.ts"() {
    "use strict";
    ...
    getCoreJsVersion = (corejsPkgPath) => {
      try {
        const rawJson = import_node_fs9.default.readFileSync(corejsPkgPath, "utf-8");
...
function updateSourceMappingURL({
  ...
  !devtool.includes("inline") && source.includes("# sourceMappingURL")) {
    const prefix = addTrailingSlash(
      import_node_path32.default.join(publicPath, config.output.distPath[type] || "")
    );
...
var import_node_path32, import_types, pluginInlineChunk;
...
var import_node_fs10, import_node_path34, pluginServer;
      ...
      setup(api) {
        ...
        api.onBeforeBuild(async ({ isFirstCompile }) => {
          ...
          for (const publicDir of publicDirs) {
            ...
            const normalizedPath = (0, import_node_path34.isAbsolute)(name) ? name : (0, import_node_path34.join)(api.context.rootPath, name);
            if (!import_node_fs10.default.existsSync(normalizedPath)) {
              continue;
            }
            try {
              await import_node_fs10.default.promises.cp(normalizedPath, api.context.distPath, {
                recursive: true,
...
var import_node_fs11, import_node_inspector, import_node_path35, import_core9, stopProfiler, pluginRspackProfile;
  ...
  "src/plugins/rspackProfile.ts"() {
    ...
    stopProfiler = (output, profileSession) => {
      ...
      profileSession.post("Profiler.stop", (error, param) => {
        ...
        import_node_fs11.default.writeFileSync(output, JSON.stringify(param.profile));
      });
    };
    pluginRspackProfile = () => ({
      ...
      async setup(api) {
        ...
        const onStart = () => {
          const profileDir = import_node_path35.default.join(api.context.distPath, profileDirName);
          const traceFilePath = import_node_path35.default.join(profileDir, "trace.json");
          if (!import_node_fs11.default.existsSync(profileDir)) {
        ...
        api.onAfterBuild(async ({ stats }) => {
          const loggingFilePath = import_node_path35.default.join(
          ...
          if (enableLogging && stats) {
            ...
            import_node_fs11.default.writeFileSync(loggingFilePath, JSON.stringify(logging));
          }
        });
        api.onExit(() => {
          if (enableProfileTrace) {
          ...
          const profileDir = import_node_path35.default.join(api.context.distPath, profileDirName);
          const cpuProfilePath = import_node_path35.default.join(profileDir, "jscpuprofile.json");
...
var import_node_path36, commonOpts, getEnvDir;
var init_init = __esm({
  "src/cli/init.ts"() {
    "use strict";
    import_node_path36 = __toESM(require("path"));
    init_config();
    init_helpers();
    init_loadEnv();
    init_logger();
    ...
    getEnvDir = (cwd, envDir) => {
      if (envDir) {
        return import_node_path36.default.isAbsolute(envDir) ? envDir : import_node_path36.default.resolve(cwd, envDir);
      }
      return cwd;
    };
  }
